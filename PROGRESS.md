...
> 마지막 업데이트: 2026-06-25 — ⑭ 풀이 결과 저장 **14-1~14-3 완료·배포 ✓** (학생 이름 입력 + 서버 채점 기록). 출제 품질 코드 검열도 적용. **다음: 14-4(출제자 화면 "이 반 풀이 결과" 보기)**.
...
- [ ] 14-4(웹): 출제자 화면에 **"이 반 풀이 결과"** 보기(누가·몇 회 만에·통과 여부) ← **다음에 여기부터**.
