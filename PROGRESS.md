...
> 마지막 업데이트: 2026-06-25 — ⑬ 수업 묶음(반마다 여러 장+무작위 출제) 코드 완료, **데스크톱에서 13-4 배포부터 이어서**.
...
## ⑬ 수업 묶음(반마다 여러 장 업로드 + 무작위 출제) — 코드 완료, 배포만 남음 ⏳ (2026-06-25)
...
- [ ] 13-4(배포) ⬅️ **데스크톱에서 여기부터 이어서 할 것**:
  1. **함수 2개 재배포**(대시보드에 코드 붙여넣기). VS Code에서 각 `index.ts` 열어 복사 → Supabase → Edge Functions → 해당 함수 → 기존 코드 지우고 붙여넣고 **Deploy**.
     - `generate-questions` , `solve-link`(이건 **Verify JWT OFF** 유지).
     - ⚠️ 한글 깨짐 방지: 채팅이 아니라 **VS Code 파일에서 직접 복사**.
  2. **웹은 이미 push됨** → Cloudflare가 자동으로 새 화면 배포. (Deployments에서 Success 확인)
     - ⚠️ **순서 주의**: 함수보다 웹이 먼저 떠 있으면, 함수 재배포 전까지 라이브 사이트의 AI출제/풀어보기가 잠깐 안 됨(화면은 batch_id를 보내는데 옛 함수는 material_id만 받음). 함수 배포하면 정상화.
  3. **실제 테스트**: 로그인 → 반 만들기 → 사진 여러 장 한 번에 업로드 → ✨AI출제(무작위 페이지 확인, 다시 누르면 다른 문제) → ▶풀어보기 → 📨학생 링크로 폰에서 풀기. 새 수업 다시 올리면 같은 링크가 오늘 것으로 바뀌는지 확인.

## 🖥️ 그 다음 후보 (13-4 배포·테스트 끝난 뒤)
