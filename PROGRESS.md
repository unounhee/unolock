...
## 🖥️ 다음 할 일 (현재 위치)
출제자 로그인 + 학원/반 관리까지 동작 확인됨. 다음 방향:
- **권장: 교재 업로드 → AI 출제(수학)** = 제품 핵심 루프로 전진.
  1) 교재 업로드: Supabase Storage + `materials` 표 권한(출제자=자기 것만) + 업로드 UI
  2) AI 출제: Claude API로 교재→수학 문제 생성 (⚠️ Claude API 키 필요 = 사용자가 직접 입력, 브라우저 노출 막기 위해 서버측/Edge Function에서 호출)
- 또는 7-3(학생·학부모 권한)을 먼저: 단 학생/학부모 화면이 아직 없어 테스트가 제한적.
