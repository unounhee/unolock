...
> 마지막 업데이트: 2026-07-01 — ⑰ **17-7a/b/c ✓ 부모 원격 잠금시각 (설정→저장→동기화)** — DB `child_settings`(부모 쓰기·학생 읽기) + 부모 화면 자녀별 "매일 잠금 시각" 설정(upsert) + 학생 앱이 홈 진입 시 서버→로컬 동기화(`setLockTime`) + 학생 화면은 읽기전용(못 바꿈). 실제 폰에서 부모가 정한 시각이 학생 폰에 반영·잠금 확인. **다음 17-7d: 허용앱 원격 설정(자녀 폰이 앱목록 서버 업로드 → 부모가 선택 → 동기화). 그 외 남은 후보: 기기관리자(삭제 방지)/제한된설정 온보딩/AI 출제 방식 교체.** (상세 ⑰ 17-7 참고)
...
- [진행중] ⑰ 2차 폰 제어(허용 앱/완전잠금) 실험 — 방향 결정: **접근성 서비스**(B2C, 공장초기화 불필요)로 가고 Device Owner는 전용폰 옵션(나중). 삭제 방지는 기기관리자+접근성 자기방어+부모PIN로. 허용앱은 **자녀 폰 실제 앱 목록에서 부모가 선택**(Option1).
  ...
  - [x] 17-3 (실험): **허용목록 차단 성공** — `BlockerService`를 "차단 모드면 허용 앱만 통과, 나머지 홈으로 튕김"으로. 안전장치: 우리 앱/홈 런처/시스템UI/설정은 항상 통과(폰 안 잠김). MainActivity set·getBlockMode / set·getAllowedPackages(SharedPreferences). `app_block_page.dart`: 차단 모드 스위치 + 허용 앱 다중 선택. 실제 폰에서 허용 앱만 열리고 나머지 튕김 확인.
  - [x] 17-4 (실험): **보상 타이머 + 미션 연동 성공** — BlockerService가 `reward_until`(시각) 전까진 전부 통과(자유시간), 지나면 다시 허용목록 차단(백그라운드 알람 불필요, 시각 비교만). MainActivity startReward(minutes)/getRewardRemaining/endReward. `app_block_page.dart` 보상 카운트다운 + 테스트 버튼(1분). `student_mission_page.dart` 통과 시 startReward(30분) 자동. 실제 폰에서 차단→보상1분(자유)→자동 재잠금 확인. **전체 사이클(공부→통과→보상시간→재잠금) 동작.**
    - **남은 후보: 기기관리자(삭제 방지) / 부모 원격 허용앱·보상시간 설정(서버 연동) / 제한된설정 온보딩.**
  - [x] 17-5: **미션 자동 차단 연동 — "항상 차단(기본상태)"** — 대표 결정: 폰은 기본이 차단(허용앱만), 미션 통과로 보상시간(자유)을 벌고 끝나면 자동 재잠금. 전엔 실험화면(`app_block_page`)에서 손으로 차단 스위치를 켜야만 사이클이 돌았음(실제 학생 흐름엔 빈틈). `student_home_page.dart`에 **홈 열릴 때 `setBlockMode(true)` 자동**(접근성 꺼졌으면 무동작이라 폰 안 잠김=안전) + **잠금 상태 카드**(접근성 꺼짐→`⚠️ 설정 필요`+[잠금 설정 열기] / 공부 시간→`🔒 허용앱만` / 보상 중→`🎁 자유 시간 m:ss` 카운트다운) + 1초 타이머·앱 복귀 시 상태 갱신(WidgetsBindingObserver). 실험 버튼은 "🔒 잠금·허용앱 설정"으로 정리. **실제 폰(SM S948N) 확인: 학생 홈 카드 표시 → adb로 접근성 ON → 카드 `🔒 공부 시간`으로 자동 전환 → 다른 앱 열면 홈으로 튕김 확인.** flutter analyze 통과.
    - 메모: 미션 통과 시 보상은 이미 연결돼 있었음(16-10/17-4). 차단을 기본 ON으로 만들어 보상시간이 비로소 실제 의미를 가짐 → **전체 사이클(공부→통과→보상→재잠금)이 실제 학생 흐름에서 자동으로 돔.**
    - 미션 통과 보상시간 **30분 → 5분**으로 변경(`student_mission_page.dart` `_finish()`, 대표 요청). "미션 하나당 짧은 보상" 개념.
    - 테스트 후 폰 잠금 해제: adb로 접근성 서비스 제거(`settings delete secure enabled_accessibility_services` + `accessibility_enabled 0`). (차단 플래그는 남아도 접근성 꺼지면 무동작=폰 자유.)

  - [x] **17-6 완료 ✓ (2026-07-01): "모든 미션 완료 → 학부모 설정 잠금시각까지 자유"** — 대표 설계 확정. 미션은 N개(승인된 반마다 오늘 미션 1개). 미션 *하나* 통과=5분 짧은 자유(17-5에서 완료), **모든 미션 통과=학부모가 정한 "매일 잠금시각"(예: 23:00)까지 자유 → 그 시각에 자동 재잠금.** 두 단계로 쪼갬: 17-6a(로컬 기계장치, 서버 무관) → 17-6b(진짜 트리거, 서버).
    - 구조 결정(중요): **잠금 시각은 서버(부모 입력)지만 자녀 폰 로컬에도 반드시 사본 저장** → 접근성 서비스(BlockerService)는 *로컬만* 읽어 **인터넷 없어도 잠김 보장**. 폰이 온라인일 때 서버→로컬 동기화. 그래서 **로컬 enforcement를 먼저(17-6a), 서버 입력/동기화는 다음(17-7)**.
    - 구현 메모(로컬): 새 저장값=**"매일 잠금시각(hour:minute)"** 로컬 1개. "모든 미션 완료" 판단=학생의 승인 반들 최신 batch를 *오늘* 다 passed 했는지(attempts에 student_id+batch_id+passed 이미 기록 중). 보상은 이미 절대시각(`reward_until`) 비교라, 모든 미션 완료 시 **reward_until=오늘 23:00**으로 설정하면 끝(23:00에 자동 재잠금). 네이티브엔 `setLockTime/getLockTime`(+필요시 `startRewardUntil(epochMs)`)만 추가. 지금은 잠금설정 화면에서 시각 선택(부모 대역).
  ...
  - [진행중] **17-7 (서버): 부모 원격 설정** — DB 표(부모가 자녀별 잠금시각/허용앱/보상 설정) + 부모 화면 + 자녀 폰 동기화(서버→로컬). 잠그는 코드는 안 바뀜(이미 로컬을 읽음). 후보 #2 "부모 원격 설정"과 동일. **배경(대표 지적): 지금 잠금시각·허용앱 설정이 학생 화면에 있는 건 로컬 enforcement 테스트용 "부모 대역"일 뿐 → 17-7에서 부모 계정으로 옮기고 학생은 못 바꾸게.**
    ...
    - [x] **17-7b ✓ (2026-07-01): 부모 화면에서 잠금시각 설정** — `parent_home_page.dart` 자녀 카드에 "🌙 매일 잠금 시각" 줄 + [정하기/바꾸기](showTimePicker) → `child_settings` upsert(student_id 충돌 시 갱신). `_load`가 자녀별 현재 lock_hour/minute도 함께 조회. 실제 폰(부모 로그인)에서 시각 저장·새로고침 후 유지 확인.
    - [x] **17-7c ✓ (2026-07-01): 자녀 폰 동기화 + 학생 화면 읽기전용** — 학생 홈 `_setupLockBaseline`(홈 진입·앱 복귀 시)에서 `_syncParentSettings`: 자기 child_settings(lock_hour/minute) 읽어 네이티브 `setLockTime`으로 로컬 복사(서버→로컬; 오프라인이면 마지막 값 유지). `app_block_page`의 잠금시각 카드를 **읽기전용**으로(부모가 정함 표시, 학생 시각변경·"모든미션완료 테스트" 버튼 제거 → 학생 편법 차단). 실제 폰: 부모가 정한 시각이 학생 잠금설정 화면에 뜨고 못 바꾸는 것 확인. flutter analyze 통과.
