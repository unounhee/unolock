...
> 마지막 업데이트: 2026-06-28 — ⑰ **17-5 미션 자동 차단 연동("항상 차단" 기본상태) ✓** (학생 홈 열면 차단 자동 ON + 잠금 상태 카드, 실제 폰에서 카드 전환·앱 튕김 확인). 전체 사이클(공부→통과→보상→재잠금)이 실제 학생 흐름에서 자동으로 돔. 보상 5분으로 조정. **다음: 17-6(로컬) "모든 미션 완료→학부모 잠금시각(예 23:00)까지 자유" — 잠금시각은 서버지만 자녀 폰 로컬에 사본 저장해 인터넷 무관 잠금. 이후 17-7(서버) 부모 원격 설정.** (상세 ⑰ 17-6/17-7 참고)
...
- [진행중] ⑰ 2차 폰 제어(허용 앱/완전잠금) 실험 — 방향 결정: **접근성 서비스**(B2C, 공장초기화 불필요)로 가고 Device Owner는 전용폰 옵션(나중). 삭제 방지는 기기관리자+접근성 자기방어+부모PIN로. 허용앱은 **자녀 폰 실제 앱 목록에서 부모가 선택**(Option1).
  ...
  - [ ] **17-6 (다음, 로컬): "모든 미션 완료 → 학부모 설정 잠금시각까지 자유"** — 대표 설계 확정. 미션은 N개(승인된 반마다 오늘 미션 1개). 미션 *하나* 통과=5분 짧은 자유(17-5에서 완료), **모든 미션 통과=학부모가 정한 "매일 잠금시각"(예: 23:00)까지 자유 → 그 시각에 자동 재잠금.**
    - 구조 결정(중요): **잠금 시각은 서버(부모 입력)지만 자녀 폰 로컬에도 반드시 사본 저장** → 접근성 서비스(BlockerService)는 *로컬만* 읽어 **인터넷 없어도 잠김 보장**. 폰이 온라인일 때 서버→로컬 동기화. 그래서 **로컬 enforcement를 먼저(17-6), 서버 입력/동기화는 다음(17-7)**.
