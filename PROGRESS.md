...
> 마지막 업데이트: 2026-06-30 — ⑰ **17-6a 로컬 잠금시각 기계장치 ✓** (네이티브 setLockTime/getLockTime/startRewardUntil + 잠금설정 화면 "🌙 매일 잠금 시각" 카드. 서버·RLS 무변경. 실제 폰에서 시각 설정→"모든 미션 완료(테스트)"→자유 시간 켜짐 확인. 자동 재잠금 최종 눈확인만 남음). **다음: 17-6b(서버) record-mission이 `all_missions_done` 반환 → 미션 다 통과 시 `startRewardUntil(오늘 잠금시각)` 자동. 그다음 17-7 부모 원격 설정.** (상세 ⑰ 17-6 참고)
...
- [진행중] ⑰ 2차 폰 제어(허용 앱/완전잠금) 실험 — 방향 결정: **접근성 서비스**(B2C, 공장초기화 불필요)로 가고 Device Owner는 전용폰 옵션(나중). 삭제 방지는 기기관리자+접근성 자기방어+부모PIN로. 허용앱은 **자녀 폰 실제 앱 목록에서 부모가 선택**(Option1).
  ...
  - [진행중] **17-6 (로컬): "모든 미션 완료 → 학부모 설정 잠금시각까지 자유"** — 대표 설계 확정. 미션은 N개(승인된 반마다 오늘 미션 1개). 미션 *하나* 통과=5분 짧은 자유(17-5에서 완료), **모든 미션 통과=학부모가 정한 "매일 잠금시각"(예: 23:00)까지 자유 → 그 시각에 자동 재잠금.** 두 단계로 쪼갬: 17-6a(로컬 기계장치, 서버 무관) → 17-6b(진짜 트리거, 서버).
    ...
    - [ ] **17-6b (다음, 서버): 진짜 트리거 연결** — `record-mission`(이미 service_role+student_id 앎)이 "오늘 승인 반 최신 batch 다 passed?"를 계산해 `all_missions_done` 반환 → 미션 통과 시(`student_mission_page._finish`) 다 끝났으면 `startReward(5분)` 대신 `startRewardUntil(오늘 잠금시각)`. "오늘" 경계는 KST 기준. 함수 1개 재배포 필요(Verify JWT ON 유지). 잠금시각 미설정이면 5분 보상 유지.
