...
## 🖥️ 다음 할 일 (현재 위치)
출제자 쪽 + 학생 풀이 루프(미리보기)까지 웹에서 동작. 다음 후보:
- **실제 학생 접속 경로**: 학생 계정/반 등록 + 학생이 자기 "오늘의 미션"을 푸는 화면(7-3 권한 포함),
  또는 공유 링크 방식. → 그래야 대표님 실제 과외 학생에게 테스트 가능.
- **풀이 결과 저장**: `attempts`/`questions`/`answers`에 기록(통과 여부·재시험 횟수 등 → 정보 비대칭 데이터).
